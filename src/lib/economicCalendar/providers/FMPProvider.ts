...
export class FMPProvider implements EconomicCalendarProvider {
  readonly name: ProviderName = "fmp";

  async fetchCalendar(options?: CalendarFetchOptions): Promise<EconomicCalendarResponse> {
    const apiKey = process.env.FMP_API_KEY;

    if (!apiKey) {
      console.warn("[FMPProvider] FMP_API_KEY not set.");
      return {
        events: [],
    ...
    try {
      ...
      const events: EconomicEvent[] = rawEvents.map((e, idx) => {
        ...
        const timePart = dateStr.includes("T")
          ? dateStr.split("T")[1]?.substring(0, 5) ?? "00:00"
          : "00:00";
        const datePart = dateStr.split("T")[0] ?? dateStr;

        return {
          id: `fmp-${idx}-${datePart}-${(e.event ?? "")
            .replace(/\s+/g, "-")
            .substring(0, 30)}`,
          date: datePart,
          time: timePart,
      ...
      console.error("[FMPProvider] fetch failed:", error);
      return {
        events: [],
        lastSync: new Date().toISOString(),
        source: "unavailable",
...
function computeStats(events: EconomicEvent[]): CalendarStats {
  ...
  return {
    ...
    usdEventsToday: events.filter(
      (e) => e.currency === "USD" && e.date === todayStr
    ).length,
    marketStatus: "Active",
  };
}
