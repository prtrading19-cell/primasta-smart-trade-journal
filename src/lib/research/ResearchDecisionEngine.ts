...
import { DEFAULT_DECISION_THRESHOLDS, type DecisionThresholds } from "@/config/decisionThresholds";
import {
  resolveWeights,
  ...
  type SourceWeights,
} from "@/lib/decisionWeightResolver";

export function calculateResearchDecision(
  ...
  score = adjustScoreForAlignment(score, overallAlignment, weights.alignmentMultiplier);
  score = adjustScoreForConflict(score, conflictScore, weights.conflictPenalty);

  const techSetupBonus = input.technicalBias.setupPresent ? 2 : 0;
  const extremeRisks = input.institutionalFlow.concentrationRisks.filter(r => r.severity === "Extreme");
  const highRisks = input.institutionalFlow.concentrationRisks.filter(r => r.severity === "High");
  const instCrowdingPenalty = extremeRisks.length * 5 + highRisks.length * 2;
  ...
  const riskRating = deriveRiskRating(conflictScore, confidence, extremeRisks.length);
...
function deriveRiskRating(conflict: number, confidence: number, extremeRisks: number): RiskRating {
...
function deriveDecisionQuality(
  ...
  if (confidence < 30 || conflict > 60 || alignment < 30) {
    return "Low";
  }
...
function calculateOverallConfidence(input: ResearchDecisionInput): number {
...
function gatherContributors(input: ResearchDecisionInput): Contributor[] {
...
function gatherConcentrationRisks(input: ResearchDecisionInput): string[] {
  const risks: string[] = [];
...
function generateSummary(
  ...
  const parts: string[] = [];
  const assetLabel = asset ? asset.toUpperCase() : "Asset";

  if (researchBias) {
    parts.push(`Institutional research bias: ${researchBias}.`);
  }

  parts.push(`${assetLabel} decision: ${decision} with ${bias.toLowerCase()} bias.`);
