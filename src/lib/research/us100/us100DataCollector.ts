...
export async function collectUS100FullDataset(): Promise<EnrichedUS100Dataset> {
  ...
  return enrichUnavailableProviders(dataset);
}
...
function enrichUnavailableProviders(dataset: US100FullDataset): EnrichedUS100Dataset {
  ...
  const enriched: EnrichedUS100Dataset = {
    ...dataset,
    index: dataset.index,
